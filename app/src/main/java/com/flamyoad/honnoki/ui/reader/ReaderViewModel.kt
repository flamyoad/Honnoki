...
class ReaderViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private val pageList = MutableLiveData<List<Page>>()
    fun pageList(): LiveData<List<Page>> = pageList

    var currentChapterId: Long = -1L
    ...
    fun fetchManga(chapterId: Long, addToFront: Boolean = false) {
        ...
        fetchMangaJob = viewModelScope.launch(Dispatchers.IO) {
            ...
            if (result is State.Success) {
                ...
                if (addToFront) {
                    existingList.addAll(0, pagesWithChapterId)
                } else {
                    existingList.addAll(pagesWithChapterId)
                }
    ...
    fun loadPreviousChapter() {
        viewModelScope.launch(Dispatchers.IO) {
            val chapter = db.chapterDao().getPreviousChapter(overviewId, currentChapterId)!!
    ...
    fun loadNextChapter() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            Log.d("log2", "Chapter loaded for chapter: ${chapterId}")
        }
    }
}